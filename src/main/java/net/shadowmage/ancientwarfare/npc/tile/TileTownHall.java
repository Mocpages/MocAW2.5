...
public class TileTownHall extends TileEntity implements IOwnable, IInventory, IInteractableTile
{
	...
	public void grantLand(NpcPlayerOwned npc, ItemStack item) {
		...
		ItemNPCSettings.getSettingsFor(item, viewSettings,this.worldObj);

		BlockPosition pos1 = viewSettings.getPos1();
		BlockPosition pos2 = viewSettings.getPos2();
		LandGrant grant = new LandGrant(pos1, pos2, viewSettings.getCorvee(), viewSettings.getRent(), viewSettings.getTithe(),npc, this);
		for(LandGrant g : lands) {
			if(g.collidesWith(grant)){
				npc.setCustomNameTag("COllision Detected!");
				return;
			}
		}
		lands.add(grant);
	...
	public ArrayList<LandGrant> getLands(){
		return lands;
	}
}
